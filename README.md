# nsmp_basic_api_connector

С версии 1.5.3:
- терминология NSD (Naumen Service Desk), заменена на терминологию платформы NSMP (Naumen Service Management Platform)
- groupId поменялся на `ru.kazantsev.nsmp`

## Что это:

Библиотека предоставляет имплементацию базового REST API NSD. Писалась под версию 4.20.
Имплементированные методы описаны в оф. документации, плюс некоторые неописанные методы группы smpsync, предназначенные для суперпользователя.
Оф. документация naumen: https://www.naumen.ru/docs/sd/415/NSD_manual.htm#RESTful/REST_API_method.htm?Highlight=REST
...
данные является размещение конфигурационного файла connector_params.json по адресу
{user.home}/.nsmp_sdk/conf/connector_params.json,
...
| installations[].ignoreSSL | Игнорировать SSL          | boolean         | Признак необходимости игнорировать SSL.                                                                                                                                                                        |
...
инсталляции, в таком случае легче всего реализовывать имплементацию кастомных методов через методы Connector.execPost() и Connector.execGet().
Авторизация в таком случае будет так же будет подготовлена, как и в случае с оригинальной библиотекой. Это простой
способ подключить ваш java проект к NSMP.

## Как использовать в NSMP:

При необходимости классы из пакета ru.ekazantsev.nsmp.basic_api_connector данной библиотеки можно собрать
в один .groovy файл и разместить его в NSMP как модуль.
Библиотека писалась с учетом такого назначения (используемые зависимости есть и в NSMP).

## Примеры создания коннектора:

1. Базовое создание, сходит в путь хранения конфигурационного файла по умолчанию (`{user.home}/.nsmp_sdk/conf/connector_params.json`), заберет оттуда данные по инсталляции с переданным ID, создаст все автоматически:

```groovy
ConnectorParams params = ConnectorParams.byConfigFile('PUBLIC_TEST')
Connector connector = new Connector(params)
//далее вызываем методы connector, какие нужны
```

2. Создание с нестандартным путем к конфигурационному файлу:

```groovy
ConnectorParams params = ConnectorParams.byConfigFileInPath('PUBLIC_TEST', "C:/my_params.json")
Connector connector = new Connector(params)
```

3. Полностью мануальное создание коннектора: 

```groovy
ConnectorParams params = new ConnectorParams(
        "MY_INSTALLATION", //ID инсталляции
        "https", //схема подключения
        "my.sd.ru", //hostname инсталляции
        "sd234gd-dfg2342-2342dg-gdr23g2", //ключ доступа к инсталляции
        false //игнор SSL
)
Connector connector = new Connector(params)
```
